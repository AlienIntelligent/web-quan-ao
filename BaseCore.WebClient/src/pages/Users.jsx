...
const Users = () => {
    ...
    const [formData, setFormData] = useState({
        ...
        phone: '', contact: '', position: '', userType: 0, isActive: true,
    });
    ...
    const openModal = (user = null) => {
        if (user) {
            ...
            setEditingUser(null);
            setFormData({ username: '', password: '', name: '', email: '', phone: '', contact: '', position: '', userType: 0, isActive: true });
        }
        setError('');
        setShowModal(true);
    };
        ...
        try {
            if (editingUser) {
                const updateData = {
                    ...
                    contact: formData.contact, position: formData.position,
                                            ...
                                            {users.length === 0 ? (
                                                ...
                                                users.map(user => (
                                                    <tr key={user.id}>
                                                        ...
                                                        <td>{user.position}</td>
                                                        <td>
                                                            <small>{user.created ? new Date(user.created).toLocaleDateString('vi-VN') : '-'}</small>
                                                        </td>
                                                        <td>
                                                            <span className={`badge ${user.userType === 1 ? 'badge-danger' : 'badge-info'}`}>
                                                                {user.userType === 1 ? 'Quản trị' : 'Khách hàng'}
                                                            </span>
                                                        </td>
                                                        <td>
                            ...
                            <form onSubmit={handleSubmit}>
                                <div className="modal-body">
                                    ...
                                    <div className="row">
                                        ...
                                        <div className="col-md-6">
                                            <div className="form-group">
                                                <label>Mật khẩu {editingUser && '(để trống để giữ nguyên)'}</label>
                                    ...
                                    <div className="row">
                                        <div className="col-md-6">
                                            <div className="form-group">
                                                <label>Chức vụ</label>
                                                <input type="text" className="form-control" value={formData.position}
                                    ...
                                    {editingUser && (
                                        <div className="form-group">
                                            <div className="custom-control custom-switch">
                                                <input type="checkbox" className="custom-control-input" id="isActiveUser"
                                                    checked={formData.isActive}
                                                    onChange={(e) => setFormData({ ...formData, isActive: e.target.checked })} />
                                                <label className="custom-control-label" htmlFor="isActiveUser">Kích hoạt tài khoản</label>
                                            </div>
                                        </div>
                                    )}
                                </div>
