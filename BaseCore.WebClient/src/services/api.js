...
export const wishlistApi = {
  ...
  add: (productId, variantId) =>
    api.post("/wishlist/add", { productId, variantId }),
  remove: (productId, variantId) =>
    api.delete("/wishlist/remove", { params: { productId, variantId } }),
  check: (productId, variantId) =>
    api.get("/wishlist/check", { params: { productId, variantId } }),
...
export const shippingApi = {
  ...
  confirm: (id) => api.post(`/shippings/${id}/confirm`),
};
...
export const cartStorage = {
  ...
  addItem: (product, quantity = 1) => {
    ...
    const existing = items.find(
      (x) => x.productId === product.id && (x.variantId || 0) === variantId,
    );
    if (existing) {
